...
import { IngresoFormData, CategoriaIngreso } from "../types/ingreso.types";
import { CATEGORIAS, CATEGORIAS_INGRESO } from "./constants";
...
export interface ValidationResultIngreso {
  isValid: boolean;
  errors: Partial<Record<keyof IngresoFormData, string>>;
}

export const validarFormularioIngreso = (
  data: IngresoFormData
): ValidationResultIngreso => {
  const errors: Partial<Record<keyof IngresoFormData, string>> = {};
  ...
  if (!data.categoria) {
    errors.categoria = "La categoría es obligatoria";
  } else if (!CATEGORIAS_INGRESO.includes(data.categoria as CategoriaIngreso)) {
    errors.categoria = "La categoría seleccionada no es válida";
  }
